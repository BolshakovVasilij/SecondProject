...
* Команда **git branch название ветки** - создание ветки.  Название ветки может представлять собой следующую запись: указатель (например feature)/описание-решаемой-задачи.  
...
* Команда **git push** - отправить локальную ветку в удаленный репозиторий.
* Команда **git pull** - скачать изменения из удаленного репозитория, сначала необходимо перейти в требуюмую ветку. Как правило, git pull - первая команда, которую  вводит разработчик, как только открывает код проекта, чтобы начать с ним работать.
